...
class Event {
    ...
    allTickets() {
        return "All Tickets: " + this.availableTickets.map((ticket, index) => 
            `${index + 1}. ${ticket.name} ($${ticket.price})`).join(" ");
    }

    // --- 16 ---
    searchTickets(lower, upper) {
        const eligibleTickets = this.availableTickets.filter(ticket => 
            ticket.price >= lower && ticket.price <= upper);

        if (eligibleTickets.length === 0) {
            return "No tickets available";
        }

        return `Eligible Tickets (<em>$${lower} - $${upper}</em>): ` + 
        eligibleTickets.map((ticket, index) => 
            `${index + 1}. ${ticket.ticketName} ($${ticket.price})`).join(" ");
    }
    ...
    getCheapestTicket() {
        ...
        const cheapestTicket = this.availableTickets.reduce((min, ticket) => 
...
console.log("Test case 1: ", eventObj3.searchTickets(0, 250)); // returns eligible tickets priced $0-$250
